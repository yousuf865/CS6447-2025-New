...
class Jpeg(KaitaiStruct):
    ...
    * EOI
        ...
        def _read(self):
            ...
            if  ((self.marker != Jpeg.Segment.MarkerEnum.soi) and (self.marker != Jpeg.Segment.MarkerEnum.eoi)) :
                ...
                if _on == Jpeg.Segment.MarkerEnum.app0:
                    ...
                    self._raw_data = self._io.read_bytes(self.length - 2)
                    _io__raw_data = KaitaiStream(BytesIO(self._raw_data))
                    self.data = Jpeg.SegmentSofData(_io__raw_data, self, self._root)
                elif _on == Jpeg.Segment.MarkerEnum.sof1:
                    pass
                    self._raw_data = self._io.read_bytes(self.length - 2)
                    _io__raw_data = KaitaiStream(BytesIO(self._raw_data))
                    self.data = Jpeg.SegmentSofData(_io__raw_data, self, self._root)
                elif _on == Jpeg.Segment.MarkerEnum.sof2:
                    pass
                    self._raw_data = self._io.read_bytes(self.length - 2)
                    _io__raw_data = KaitaiStream(BytesIO(self._raw_data))
                    self.data = Jpeg.SegmentSofData(_io__raw_data, self, self._root)
                elif _on == Jpeg.Segment.MarkerEnum.sof3:
                    pass
                    self._raw_data = self._io.read_bytes(self.length - 2)
                    _io__raw_data = KaitaiStream(BytesIO(self._raw_data))
                    self.data = Jpeg.SegmentSofData(_io__raw_data, self, self._root)
    ...
    class SegmentSofData(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Jpeg.SegmentSofData, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            for i in range(self.num_components):
                self.components.append(Jpeg.SegmentSofData.Component(self._io, self, self._root))
        ...
        class Component(KaitaiStruct):
            def __init__(self, _io, _parent=None, _root=None):
                super(Jpeg.SegmentSofData.Component, self).__init__(_io)
                self._parent = _parent
                self._root = _root
                self._read()
