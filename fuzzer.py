...
class Fuzzer:
    ...
    def run_target(self, src: str, payload: str):
        try:
            ...
            if output == "" and error == "":
                output = "GENERATED BY FUZZER!!:\nEmpty Output!\nThis means the program did not exit with any errors!"
            return output, error
        except Exception as e:
            return f"Exit Status Error: {e}", f"Exit Status Error: {e}"
    ...
    def sample_name(self, bin_name: str, file_type, file_type_dict: dict, num_tests: int = 100):
        output_list = []
        unique_output_mutations = []
        hhh = {}
        summary = "No abnormal errors found!"
        max_val_bin = {
            "csv1": 257,
            "csv2": 257
        }
        input_type = "filler"
        # This part checks if the item from the list file type is in file_type_dict
        ...
        if input_type in file_type_dict.keys():
            ...
            for strat in strategies:
                for i in range(1, num_tests):
                    ...
                    output, error = self.run_target(f"binaries/{bin_name}", mutated_payload)
            ...
            for i in range (1, num_tests):
                ...
                args_mutation = [random.choice([True, False]) for _ in range(arg_amount-1)]
